...
import { int32, uint32, float64, float32, uint8 } from "../fable-library.4.0.0-theta-001/Int32.js";
...
import { fromNumber, fromInteger, int64, fromValue, uint64, op_RightShiftUnsigned, toInt, fromBits, compare } from "../fable-library.4.0.0-theta-001/Long.js";
import { decimal, getBits } from "../fable-library.4.0.0-theta-001/Decimal.js";
...
import { BigInteger } from "../fable-library.4.0.0-theta-001/BigInt/z.js";
...
function cacheGetOrAdd<$a>(typ: any, f: (arg0: any, arg1: Array<uint8>) => void): (arg0: $a, arg1: Array<uint8>) => void {
    let matchValue;
    let outArg = null;
    matchValue = [tryGetValue(serializerCache, fullName(typ), new FSharpRef((): (arg0: any, arg1: Array<uint8>) => void => outArg, (v: (arg0: any, arg1: Array<uint8>) => void): void => {
        outArg = curry(2, v);
    })), outArg];
    if (matchValue[0]) {
        return (arg: $a): (arg0: Array<uint8>) => void => {
            const clo = matchValue[1](arg);
            return (arg_1: Array<uint8>): void => {
                clo(arg_1);
            };
        };
    }
        ...
        return (arg_2: $a): (arg0: Array<uint8>) => void => {
            const clo_1 = partialApply(1, f, [arg_2]);
            return (arg_3: Array<uint8>): void => {
                clo_1(arg_3);
            };
        };
    }
...
function write64bitNumber(b1: uint8, b2: uint8, b3: uint8, b4: uint8, b5: uint8, b6: uint8, b7: uint8, b8: uint8, out: Array<uint8>): void {
...
function writeSignedNumber(bytes: Array<uint8>, out: Array<uint8>): void {
    if (isLittleEndian()) {
...
function writeUInt64(n: uint64, out: Array<uint8>): void {
...
function writeInt64(n: int64, out: Array<uint8>): void {
...
function writeByte(b: uint8, out: Array<uint8>): void {
    if (b < 128) {
        void (out.push(b));
    }
    else {
...
function writeSingle(n: float32, out: Array<uint8>): void {
    void (out.push(202));
    writeSignedNumber(Array.from(getBytesSingle(n)), out);
}

function writeDouble(n: float64, out: Array<uint8>): void {
    void (out.push(203));
    writeSignedNumber(Array.from(getBytesDouble(n)), out);
}

function writeBin(data: Array<uint8>, out: Array<uint8>): void {
...
function writeArrayHeader(len: int32, out: Array<uint8>): void {
...
function writeDecimal(n: decimal, out: Array<uint8>): void {
    ...
    for (let idx: int32 = 0; idx <= (bits.length - 1); idx++) {
        const b = bits[idx] | 0;
...
function writeArray(out: Array<uint8>, t: any, arr: any): void {
...
function writeMap(out: Array<uint8>, keyType: any, valueType: any, dict: any): void {
    let value: int32, n: uint32, b: uint8, value_1: uint32, b_1: uint8, value_1_1: uint32, b_2: uint8, value_2: uint32, b_3: uint8;
...
function writeSet(out: Array<uint8>, t: any, set$: any): void {
...
function writeUnion(out: Array<uint8>, tag: int32, types: Array<any>, vals: Array<any>): void {
    if (vals.length === 0) {
        void (out.push(144 + 1));
        ...
        if (vals.length === 1) {
            ...
            for (let i: int32 = 0; i <= (vals.length - 1); i++) {
...
export function writeObject(x: any, t: any, out: Array<uint8>): void {
    if (x == null) {
        void (out.push(192));
    }
    else {
        let matchValue;
        let outArg = null;
        matchValue = [tryGetValue(serializerCache, fullName(t), new FSharpRef((): (arg0: any, arg1: Array<uint8>) => void => outArg, (v: (arg0: any, arg1: Array<uint8>) => void): void => {
            outArg = curry(2, v);
        })), outArg];
        if (matchValue[0]) {
            matchValue[1](x)(out);
            ...
            const fieldTypes = map((x_1: any): any => x_1[1], getRecordElements(t, true));
            cacheGetOrAdd(t, (x_2: any, out_2: Array<uint8>): void => {
                const out_3 = out_2;
            ...
            if (equals(tDef, list_type(obj_type))) {
                ...
                cacheGetOrAdd(t, (x_8: any, out_10: Array<uint8>): void => {
                    writeArray(out_10, elementType_1, x_8);
                })(x)(out);
            }
            else if (equals(tDef, option_type(obj_type))) {
                cacheGetOrAdd(t, (x_9: any, out_11: Array<uint8>): void => {
                    const opt = x_9;
                ...
                cacheGetOrAdd(t, (x_10: any, out_12: Array<uint8>): void => {
                ...
                cacheGetOrAdd(t, (x_11: any, out_13: Array<uint8>): void => {
...
addToDict(serializerCache, "System.Byte", (x: any): (arg0: Array<uint8>) => void => ((out: Array<uint8>): void => {
    writeByte(x, out);
}));

addToDict(serializerCache, "System.SByte", (x: any): (arg0: Array<uint8>) => void => ((out: Array<uint8>): void => {
    writeByte(x & 0xFF, out);
}));

addToDict(serializerCache, "Microsoft.FSharp.Core.Unit", (_arg: any): (arg0: Array<uint8>) => void => ((out: Array<uint8>): void => {
    void (out.push(192));
}));

addToDict(serializerCache, "System.Boolean", (x: any): (arg0: Array<uint8>) => void => ((out: Array<uint8>): void => {
    void (out.push(x ? 195 : 194));
}));

addToDict(serializerCache, "System.Char", (x: any): (arg0: Array<uint8>) => void => ((out: Array<uint8>): void => {
    let len: int32, n: uint32, b: uint8, value: uint32, b_1: uint8, value_1: uint32, b_2: uint8, value_2: uint32, b_3: uint8;
    const out_1 = out;
    const str_1 = Array.from(get_UTF8().getBytes(x));
    if (str_1.length < 32) {
...
addToDict(serializerCache, "System.String", (x: any): (arg0: Array<uint8>) => void => ((out: Array<uint8>): void => {
    let len: int32, n: uint32, b: uint8, value: uint32, b_1: uint8, value_1: uint32, b_2: uint8, value_2: uint32, b_3: uint8;
    const out_1 = out;
    const str_1 = Array.from(get_UTF8().getBytes(x));
    if (str_1.length < 32) {
...
addToDict(serializerCache, "System.Int32", (x: any): (arg0: Array<uint8>) => void => ((out: Array<uint8>): void => {
    writeInt64(fromInteger(x, false, 2), out);
}));

addToDict(serializerCache, "System.Int16", (x: any): (arg0: Array<uint8>) => void => ((out: Array<uint8>): void => {
    writeInt64(fromInteger(x, false, 1), out);
}));

addToDict(serializerCache, "System.Int64", (x: any): (arg0: Array<uint8>) => void => ((out: Array<uint8>): void => {
    writeInt64(x, out);
}));

addToDict(serializerCache, "System.UInt32", (x: any): (arg0: Array<uint8>) => void => ((out: Array<uint8>): void => {
    writeUInt64(fromInteger(x, true, 6), out);
}));

addToDict(serializerCache, "System.UInt16", (x: any): (arg0: Array<uint8>) => void => ((out: Array<uint8>): void => {
    writeUInt64(fromInteger(x, true, 5), out);
}));

addToDict(serializerCache, "System.UInt64", (x: any): (arg0: Array<uint8>) => void => ((out: Array<uint8>): void => {
    writeUInt64(x, out);
}));

addToDict(serializerCache, "System.Single", (x: any): (arg0: Array<uint8>) => void => ((out: Array<uint8>): void => {
    writeSingle(x, out);
}));

addToDict(serializerCache, "System.Double", (x: any): (arg0: Array<uint8>) => void => ((out: Array<uint8>): void => {
    writeDouble(x, out);
}));

addToDict(serializerCache, "System.Decimal", (x: any): (arg0: Array<uint8>) => void => ((out: Array<uint8>): void => {
    writeDecimal(x, out);
}));
...
addToDict(serializerCache, "System.Numerics.BigInteger", (x: any): (arg0: Array<uint8>) => void => ((out: Array<uint8>): void => {
    let copyOfStruct: BigInteger;
    writeBin((copyOfStruct = x, toByteArray(copyOfStruct)), out);
}));

addToDict(serializerCache, "System.Guid", (x: any): (arg0: Array<uint8>) => void => ((out: Array<uint8>): void => {
    let copyOfStruct: string;
    writeBin((copyOfStruct = x, guidToArray(copyOfStruct)), out);
}));

addToDict(serializerCache, "System.DateTime", (x: any): (arg0: Array<uint8>) => void => ((out: Array<uint8>): void => {
    const out_1 = out;
    const dto = x;
...
addToDict(serializerCache, "System.DateTimeOffset", (x: any): (arg0: Array<uint8>) => void => ((out: Array<uint8>): void => {
    let copyOfStruct: number;
    const out_1 = out;
    const dto = x;
    void (out_1.push(144 + 2));
    writeInt64(getTicks(dto), out_1);
...
addToDict(serializerCache, "System.DateOnly", (x: any): (arg0: Array<uint8>) => void => ((out: Array<uint8>): void => {
    let n: uint32, b: uint8, value: uint32, b_1: uint8, value_1: uint32, b_2: uint8, value_2: uint32, b_3: uint8;
    ((n = (dayNumber(x) >>> 0), (b = ((value = (n >>> 24), value & 0xFF)), (b_1 = ((value_1 = (n >>> 16), value_1 & 0xFF)), (b_2 = ((value_2 = (n >>> 8), value_2 & 0xFF)), (b_3 = (n & 0xFF), (writeFormat: boolean): void => {
        const b1 = b;
        const b2 = b_1;
        const b3 = b_2;
        const b4 = b_3;
...
addToDict(serializerCache, "System.TimeOnly", (x: any): (arg0: Array<uint8>) => void => ((out: Array<uint8>): void => {
    writeUInt64(fromValue(ticks(x), true), out);
}));

addToDict(serializerCache, "System.TimeSpan", (x: any): (arg0: Array<uint8>) => void => ((out: Array<uint8>): void => {
    let copyOfStruct: number;
