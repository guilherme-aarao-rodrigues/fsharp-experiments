...
import { int32, float64 } from "../fable-library.4.0.0-theta-001/Int32.js";
...
export const negJint = Parsimmon_map((tupledArg: [string, float64]): float64 => (-tupledArg[1]), Parsimmon_seq2(Parsimmon_str("-"), jint));
...
export const jnumber = Parsimmon_map((arg: float64): Json => (new Json(0, arg)), Parsimmon_choose(ofArray([jfloat, jint, negJint])));
...
export const stringLiteral = (() => {
    const escape = Parsimmon_map((_arg: string): string => {
        if (_arg === "b") {
            return "\b";
        }
        else if (_arg === "f") {
    ...
    const anyCharSnippet = Parsimmon_concat(Parsimmon_many(Parsimmon_orTry(Parsimmon_map((tuple: [string, string]): string => tuple[1], Parsimmon_seq2(Parsimmon_str("\\"), escape)), Parsimmon_satisfy((c_1: string): boolean => ((c_1 !== "\"") && (c_1 !== "\\"))))));
    return Parsimmon_between(Parsimmon_str("\""), Parsimmon_str("\""), anyCharSnippet);
})();

export const jstring = stringLiteral.map((arg: string): Json => (new Json(1, arg)));

export function withWhitespace<$a>(p: any): any {
...
export function json$004080(): any {
    return Parsimmon_ofLazy((): any => Parsimmon_choose(ofArray([jvalue, Parsimmon_map((arg_1: Array<Json>): Json => (new Json(4, ofArray(arg_1))), Parsimmon_between(withWhitespace(Parsimmon_str("[")), withWhitespace(Parsimmon_str("]")), Parsimmon_seperateBy(comma, json$004080$002D1.Value))), Parsimmon_map((arg_4: Array<[string, Json]>): Json => (new Json(5, ofList(ofArray(arg_4), {
        Compare: comparePrimitives,
    }))), Parsimmon_between(withWhitespace(Parsimmon_str("{")), withWhitespace(Parsimmon_str("}")), Parsimmon_seperateBy(comma, Parsimmon_map((tupledArg: [string, string, Json]): [string, Json] => [tupledArg[0], tupledArg[2]], Parsimmon_seq3(withWhitespace(stringLiteral), withWhitespace(Parsimmon_str(":")), withWhitespace(json$004080$002D1.Value))))))])));
}
